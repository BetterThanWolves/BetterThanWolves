...
public class FCEntityWolf extends EntityWolf
{
    ...
    public boolean interact( EntityPlayer player )
    {
        ...
        if ( playerStack != null )
        {
    		if ( playerStack.itemID == FCBetterThanWolves.fcItemWolfRaw.itemID ||
    			playerStack.itemID == FCBetterThanWolves.fcItemWolfCooked.itemID )
    		{
        ...
        {
	        if ( !worldObj.isRemote && isTamed() && player.username.equalsIgnoreCase( 
	        	getOwnerName() ) && ( playerStack == null || !IsEdibleItem( playerStack ) ) )
	        {
    ...
    public boolean isBreedingItem( ItemStack stack )
    {
    	if ( stack != null && isTamed() )
    	{
    		...
    		if ( item.IsWolfFood() )
    		{
				...
				if ( iItemID != Item.rottenFlesh.itemID && 
					iItemID != FCBetterThanWolves.fcItemChocolate.itemID )
				{
					return true;
				}
    		}
    ...
    {
        if ( isTamed() && ( dataWatcher.getWatchableObjectInt( 18 ) < 20 || !IsFullyFed() ) )
        {
            OnEat( stack.getItem() );
	...
	{
		if ( worldObj.getWorldInfo().getGameType() != EnumGameType.CREATIVE && m_iInfectionCountdown < 0 )
		{
