...
public class FCBlockDirtSlab extends FCBlockSlabAttached
{
    ...
    public boolean RenderBlock( RenderBlocks renderer, int i, int j, int k )
    {
        ...
        if ( iSubtype == FCBlockDirtSlab.m_iSubtypeGrass && blockAccess.getBlockId( i, j + 1, k ) != Block.snow.blockID )
        {
    ...
    public int colorMultiplier( IBlockAccess blockAccess, int i, int j, int k )
    {
		...
		if ( iSubtype == m_iSubtypeGrass && blockAccess.getBlockId( i, j + 1, k ) != Block.snow.blockID )
		{
	        int iRed = 0;
	        int iGreen = 0;
	        int iBlue = 0;
